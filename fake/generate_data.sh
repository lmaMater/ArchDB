...
FILLING_CNT=$((FILLING_CNT < 1000 ? 1000 : FILLING_CNT))
SMALLER_FILLING_CNT=$((FILLING_CNT / 10))
...
generate_data() {
  ...
  # users ok
  psql -h db -U "${USER}" -d aoty -c "INSERT INTO users (username, role, email, creation_date, description) SELECT 'user' || gs || '_' || md5(random()::text), CASE WHEN random() < 0.80 THEN 'user'::user_role WHEN random() < 0.95 THEN 'premium_user'::user_role ELSE 'admin'::user_role END, 'user' || gs || '_' || md5(random()::text) || '@example.com', CURRENT_DATE - interval '1' day * floor(random() * 3650), 'description' FROM generate_series(1, $SMALLER_FILLING_CNT) AS gs;" -W

  # artists ok
  psql -h db -U "${USER}" -d aoty -c "INSERT INTO artists (id, name) SELECT generate_series(1, $SMALLER_FILLING_CNT), 'artist' || md5(random()::text);" -W

  # releases ok
  psql -h db -U "${USER}" -d aoty -c "INSERT INTO releases (id, title, cover, date, format) SELECT generate_series(1, $SMALLER_FILLING_CNT), 'title' || md5(random()::text), 'cover' || generate_series(1, $SMALLER_FILLING_CNT), CURRENT_DATE - interval '1' day * floor(random() * 18250), CASE WHEN random() < 0.33 THEN 'single'::release_format WHEN random() < 0.66 THEN 'ep'::release_format ELSE 'lp'::release_format END;" -W

  # streaming_services ok
  psql -h db -U "${USER}" -d aoty -c "INSERT INTO streaming_services (id, name) SELECT generate_series(1, $SMALLER_FILLING_CNT), 'service' || generate_series(1, $SMALLER_FILLING_CNT);" -W

  # genres (maybe FILLING_CNT / 10)
  psql -h db -U "${USER}" -d aoty -c "INSERT INTO genres (id, name) SELECT generate_series(1, $SMALLER_FILLING_CNT), 'genre' || generate_series(1, $SMALLER_FILLING_CNT);" -W

  # publications ok
  psql -h db -U "${USER}" -d aoty -c "INSERT INTO publications (id, name, link) SELECT generate_series(1, $SMALLER_FILLING_CNT), 'publication' || generate_series(1, $SMALLER_FILLING_CNT), 'http://example.com/' || md5(random()::text);" -W
  ...
  SELECT DISTINCT ON (release_id, artist_id)
      floor(random() * $SMALLER_FILLING_CNT) + 1 AS release_id,
      floor(random() * $SMALLER_FILLING_CNT) + 1 AS artist_id
  FROM
  ...
  SELECT DISTINCT ON (release_id, genre_id)
      floor(random() * $SMALLER_FILLING_CNT) + 1 AS release_id,
      floor(random() * $SMALLER_FILLING_CNT) + 1 AS genre_id
  FROM
      generate_series(1, $FILLING_CNT);" -W
  ...
  SELECT DISTINCT ON (artist_id, member_id)
      floor(random() * $SMALLER_FILLING_CNT) + 1 AS artist_id,
      floor(random() * $SMALLER_FILLING_CNT) + 1 AS member_id
  FROM
      generate_series(1, $FILLING_CNT);" -W
  ...
  SELECT DISTINCT ON (user_subscriber_id, user_id)
      floor(random() * $SMALLER_FILLING_CNT) + 1 AS user_subscriber_id,
      floor(random() * $SMALLER_FILLING_CNT) + 1 AS user_id
  FROM
      generate_series(1, $FILLING_CNT);" -W
  ...
  SELECT DISTINCT ON (user_subscriber_id, publication_id)
      floor(random() * $SMALLER_FILLING_CNT) + 1 AS user_subscriber_id,
      floor(random() * $SMALLER_FILLING_CNT) + 1 AS publication_id
  FROM
      generate_series(1, $FILLING_CNT);" -W
  ...
  SELECT DISTINCT ON (release_id, streaming_service_id)
      floor(random() * $SMALLER_FILLING_CNT) + 1 AS release_id,
      floor(random() * $SMALLER_FILLING_CNT) + 1 AS streaming_service_id,
